 ...
 ***********************************************************************************************
 */
CREATE TABLE IF NOT EXISTS public.text_processing_logs (
    -- 증가형 PK
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,

    -- 해시 키 (유니크)
    hash_key VARCHAR(64) NOT NULL UNIQUE,  -- SHA256 해시는 64자
...
CREATE INDEX IF NOT EXISTS idx_text_processing_logs_status 
    ON public.text_processing_logs (processing_status);

CREATE INDEX IF NOT EXISTS idx_text_processing_logs_priority 
    ON public.text_processing_logs (priority)
    WHERE processing_status = 'pending';

CREATE INDEX IF NOT EXISTS idx_text_processing_logs_error_flag
    ON public.text_processing_logs (is_error_occurred)
...
CREATE POLICY "text_processing_logs are visible to everyone" 
    ON public.text_processing_logs
    FOR SELECT
...
CREATE TRIGGER trigger_update_text_processing_logs_updated_at
    BEFORE UPDATE ON public.text_processing_logs
    FOR EACH ROW
