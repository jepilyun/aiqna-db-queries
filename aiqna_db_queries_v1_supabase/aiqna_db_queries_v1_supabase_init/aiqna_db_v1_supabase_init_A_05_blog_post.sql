...
CREATE POLICY "blog_post_processing_logs are visible to everyone" 
    ON blog_post_processing_logs FOR SELECT 
    TO authenticated, anon 
    USING (TRUE);

-- 관리 작업 (service_role만)
CREATE POLICY "Service role can manage blog_post_processing_logs" 
    ON blog_post_processing_logs FOR ALL 
    TO service_role 
    USING (TRUE) 
    WITH CHECK (TRUE);
...
CREATE TRIGGER trigger_update_blog_post_processing_logs_updated_at
    BEFORE UPDATE ON blog_post_processing_logs
    FOR EACH ROW
